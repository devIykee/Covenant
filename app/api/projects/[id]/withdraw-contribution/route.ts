...
// A backer withdraws their deposit before funds are locked (e.g. the raise is
// under the builder's minimum, or they change their mind). The custodian refunds
// every ACTIVE contribution of that backer via a real SIP-010 transfer.
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const { backer } = await req.json();

  if (!backer) return NextResponse.json({ error: "Connect your wallet to withdraw." }, { status: 400 });
  ...
  const mine = project.contributions.filter((c) => eqAddr(c.principal, backer) && c.status !== "WITHDRAWN");
  if (mine.length === 0) {
  ...
  try {
    const { txid, explorerUrl } = await transferToken(backer, total.toString(), `Covenant ${id} deposit withdrawal`);
    ...
    await db.projectStateLog.create({
      data: { projectId: id, status: project.status, txid, note: `Backer ${backer.slice(0, 8)}… withdrew deposit` },
    });
