...
// Backers (and only backers) appoint the judges for a covenant, after they've
// deposited. The builder cannot — they'd be picking their own referees. A backer
// may appoint themselves. Judges can only be set before the funds are locked.
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const { backer, judges } = await req.json();
  ...
  // Caller must be a backer who has actually deposited into this covenant.
  const isBacker = project.contributions.some((c) => eqAddr(c.principal, backer) && c.status !== "WITHDRAWN");
  if (!isBacker) {
    return NextResponse.json({ error: "Only backers who have funded can appoint judges." }, { status: 403 });
  }
  ...
  await db.projectStateLog.create({
    data: { projectId: id, status: project.status, note: `Backer ${backer.slice(0, 8)}… appointed ${incoming.length} judge(s)` },
  });
